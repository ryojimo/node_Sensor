 ...
 * @return {Object} this.data30s - 更新した this.data30s の JSON オブジェクト
 * @example
...
//  console.log( "[DataSensors.js] jsonObj = " + JSON.stringify(jsonObj) );
 ...
 * isLarge( 'sa_acc_x' );
*/
DataSensors.prototype.isLarge = function( name ){
...
/**
 * 対象のコレクション (= day ) にドキュメント (= hour ) を作成する。
 * @param {string} day - 日付。( MongoDB のコレクション名でも使用 )
 * @param {string} hour - 時間。
 * @param {Object.<string, number>} jsonObj - {センサ名:値, ...} が入った JSON オブジェクト
 * @return {void}
 * @example
 * createDoc( '2018-05-14', '08:00', {} );
*/
DataSensors.prototype.createDoc = function( day, hour, jsonObj ){
  console.log( "[DataSensors.js] createDoc()" );
  console.log( "[DataSensors.js] day     = " + day );
  console.log( "[DataSensors.js] hour    = " + hour );
  console.log( "[DataSensors.js] jsonObj = " + JSON.stringify(jsonObj) );
  ...
  MongoClient.connect( this.mongo_url, function(err, db){
    ...
    var dbo = db.db( 'sensors' );     // データベースを取得する
 ...
 * @return {void}
 * @example
 * getOneDay( '2018-05-14', 'si_bme280_temp', function( err, doc ){} );
*/
DataSensors.prototype.getOneDay = function( day, sensor, callback ){
  console.log( "[DataSensors.js] getOneDay()" );
  console.log( "[DataSensors.js] day    = " + day );
  ...
  MongoClient.connect( this.mongo_url, function(err, db){
    ...
    clo.find( {} ).toArray( function(err, docs){
      try{
        ...
        var i = 0;
        for( var key in data ){
          data[key] = docs[i].sensors[sensor];
          i++;
        }
        db.close();
        console.log( "[DataSensors.js] data = " + JSON.stringify(data) );
        callback( true, data );
      }
      catch( e ){
        console.log( "[DataSensors.js] e = " + e + " : " + e.message );
